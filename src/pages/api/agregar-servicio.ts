...
export const POST: APIRoute = async ({ request, redirect }) => {
  ...
  try {
    ...
    if (servicioExistente) {
        return redirect(`/admin?mensaje=El servicio ya existe, no se puede agregar.&tipo=error`);
    } else {
    ...
    return redirect(`/admin?mensaje=El servicio se ha agregado correctamente.&tipo=exito`);
} catch (error) {
    console.error("Error guardando el servicio:", error);
    return redirect(`/admin?mensaje=Error guardando el servicio.&tipo=error`);
}
};
